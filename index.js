...
checkbox.addEventListener("change", function (event) {
  if (this.checked) {
    ...
    document.getElementById("inst-box-htp").className =
      "inst-box inst-box-dark";
...
document.getElementById("length").addEventListener("change", changeLength);
  ...
  dbRef
    ...
    .then(async (snapshot) => {
      if (snapshot.exists()) {
        ...
        if (data.ready2) {
          alert("Sorry, someone has already joined this room");
          document.querySelector("#join-game").removeAttribute("disabled");
          document.querySelector("#room-id").removeAttribute("disabled");
          document.querySelector("#join-game").innerHTML = "Join Game";
          return;
              ...
              if ((tempData.turn === username) & (board[i][j] === "")) {
                ...
                firebase
                  ...
                  .update({
                    board: board,
                    turn:
                      tempData.turn === tempData.player1
                        ? tempData.player2
                        : tempData.player1,
                  });
